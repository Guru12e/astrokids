...
export async function POST(req) {
  try {
    ...
    // Parse date with IST fallback
    const dateObj = toDateWithISTFallback(date);

    // Prepare debug info
    const debug = {
      input_date_string: date,
      parsed_iso: dateObj.toISOString(),
      parsed_tz_offset_minutes: dateObj.getTimezoneOffset(),
      lat: latNum,
      lon: lonNum,
      node_env: process.env.NODE_ENV || "unknown",
      node_tz_env: process.env.TZ || null,
    };

    // Ascendant internals
    const ascCalc = calculateAscendantTropical(dateObj, latNum, lonNum);
    const ascTropical = ascCalc.asc;
    const gmst = ascCalc.gmst;
    const lmst = ascCalc.lmst;
    const ramc = ascCalc.ramc;
    const ascSid = (ascTropical - LAHIRI_AYANAMSHA + 720) % 360;

    debug.asc_tropical = ascTropical;
    debug.asc_sidereal = ascSid;
    debug.gmst = gmst;
    debug.lmst = lmst;
    debug.ramc = ramc;
    ...
    // Build ascendant object using your degreeToSign + getNakshatra
    const ascSignInfo = degreeToSign(ascTropical);
    const ascNak = getNakshatra(ascTropical);
    const ascendantObj = {
      Name: "Ascendant",
      full_degree: ascSid,
    ...
    all.forEach(
      (obj) => (obj.pos_from_asc = getHousePosition(obj.full_degree, ascSid))
    );

    const { sunrise, sunset } = getSunriseSunset(dateObj, latNum, lonNum);
    debug.sunrise = sunrise;
    debug.sunset = sunset;

    // Panchang using sidereal longitudes (as in your code)
    const sunSid = planets.find((p) => p.Name === "Sun").full_degree;
    const moonSid = planets.find((p) => p.Name === "Moon").full_degree;
    const [tithi, tithiNumber, paksha] = calculateTithi(sunSid, moonSid);
    const [nakshatra, nakIndex] = calculateNakshatra(moonSid);
    const [yoga, yogaIndex] = calculateYoga(sunSid, moonSid);
    const [karana, karanaIndex] = calculateKarana(tithiNumber, sunSid, moonSid);
    ...
    const panchang = {
      ...
      sunrise,
      sunset,
      ganam: calculateGanam(nakshatra) || null,
    ...
    // return everything for debugging comparison
    return NextResponse.json(
      { debug, planets: all, panchang },
      { status: 200 }
    );
  } catch (err) {
    console.error("astro-debug error:", err);

...
function ContentDesign(
  ...
  if (typeof content === "string") {
    content = content.replace(/child/g, name);
    ...
    for (const [k, v] of Object.entries(content)) {
      ...
      if (k === "name" || k === "field") {
        ...
        const replaced = v.replace(/child/g, name);
        y += 10;
    ...
    for (let i = 0; i < content.length; i++) {
      const v1 = content[i];
      if (typeof v1 === "string") {
        let replaced = v1.replace(/child/g, name);
        const height =
        ...
        v1.content = String(v1.content).replace(/child/g, name);
...
async function UltimateReport(
  ...
  ContentDesign(
    ...
    randomeColor(),
    "",
    String(con[0]).replace("child", name).replace("Child", name),
    ICONS,
    name
  );
  ...
  content[2].forEach((step, index) => {
    step = step.replace("child", name).replace("Child", name);
    let height = doc
      .font("Linotte-Regular")
      .fontSize(14)
      .heightOfString(index + 1 + ") " + step, {
...
export const generateReport = async (
  ...
  try {
    ...
    await transporter.sendMail({
      ...
      html: `<html> <head> </head> <body> <div style=" margin: auto; width: 100vw; max-width: 600px; padding-top: 20px; position: relative; "> <div style=" position: absolute; top: 0; left: 0; z-index: 0; width: 100vw; height: 100%; max-width: 600px; background: red; opacity: 0.18; "></div> <div style="z-index: 10; color: black; opacity: 1"> <div style=" width: 100%; margin-top: 40px; background-color: #210535; "> <img src="https://drive.usercontent.google.com/download?id=1MB_IKZo35iEaSzUCaZCqYT9XU39IKh8h" alt="logo" style="width: 40%; aspect-ratio: 16 / 9; margin: 0 auto" /> </div> </div> <div style="padding: 0 20px"> <div style="width: 100%;"> <h1>Dear Parents,</h1> <h3>We are excited to share ${name}'s personalized report. Astro Kids ensures that you receive insights with accuracy and care.</h3> </div> <div style="color:#210535; font-size:larger;"> <p>Your child's report is now available. Click the button below to view or download the report:</p> </div> <div style="color:#210535; font-size:larger;"> <p>If you did not request this report or need assistance, please contact our customer support immediately.<br><br> Thank You,<br> AstroKids Team </p> </div> <div style="margin-top: 70px"> <p style="font-size: 20px">Warm Regards,</p> <img src="https://drive.usercontent.google.com/download?id=1MB_IKZo35iEaSzUCaZCqYT9XU39IKh8h" alt="signature" width="100px" /> <p>The Astrokids Team</p> <a>support@astrokids.ai</a> <a href="https://astrokids.ai/" style=" display: block; color: black; text-decoration: none; margin: 10px 0; ">astrokids.ai</a> </div> </div> </div> </body> </html>`,
    });
